...
class VoiceInputGUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        # 触发键选择：显示友好名称，保存英文 ID
        hk_layout = QHBoxLayout()
        hk_layout.addWidget(QLabel("触发按键:"))
        self.hk_combo = QComboBox()
        # 填充选项：Data 存 ID，Text 存显示名
        for hk_id, display_name in HOTKEY_DISPLAY_NAMES.items():
            self.hk_combo.addItem(display_name, hk_id)
        
        # 设置当前选中项
        current_hk_id = current_config.get("hotkey", "LCtrl")
        index = self.hk_combo.findData(current_hk_id)
        if index >= 0:
            self.hk_combo.setCurrentIndex(index)
